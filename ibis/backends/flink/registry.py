...
def _cast(translator: ExprTranslator, op: ops.generic.Cast) -> str:
    ...
    if to.is_timestamp():
        ...
        if to.timezone:
            if to.timezone == "Europe/London":
                # Note (mehmet): `pytz.timezone` returns "LMT" while
                # `test_timestamp_with_timezone_literal` expects "GMT".
                timezone_abbreviation = "GMT"
            else:
                timezone_abbreviation = pytz.timezone(to.timezone)._tzname

            return (
                "CONCAT("
                f"CONVERT_TZ(CAST({arg_translated} AS STRING), 'UTC+0', '{to.timezone}')"
...
def _string_split(translator: ExprTranslator, op: ops.StringSplit) -> str:
    arg = translator.translate(op.arg)
    delimiter = translator.translate(op.delimiter)

    # TODO (mehmet): The following is a bit hacky to work around the fact that
    # Flink does not provide an operator to split string into an array.
    return f"MAP_KEYS(STR_TO_MAP(REGEXP_REPLACE({arg}, {delimiter}, '=1,'), ',', '='))"
...
def _date_delta(
    translator: ExprTranslator, op: ops.temporal.DateDelta
) -> str:
    left = translator.translate(op.left)
    right = translator.translate(op.right)
    unit = op.part.value.upper()

    return f"TIMESTAMPDIFF({unit}, CAST({right} AS TIMESTAMP), CAST({left} AS TIMESTAMP))"
...
def _timestamp_bucket(translator: ExprTranslator, op: ops.temporal.TimestampBucket) -> str:
    arg_translated = translator.translate(op.arg)
    interval_translated = translator.translate(op.interval)
...
def _timestamp_delta(translator: ExprTranslator, op: ops.temporal.TimestampDelta) -> str:
    left = translator.translate(op.left)
    right = translator.translate(op.right)
    unit = op.part.value.upper()
